...
class ConnectionTest extends TestCase {
    ...
    public function testAddTable(){
        try {
            self::$connection->executeQuery("DROP TABLE `CVStatic`.`testPHPUnit`;");
        } catch (Exception $e) {}
        
        self::$connection->executeQuery("CREATE TABLE `CVStatic`.`testPHPUnit` ( `test1` INT NOT NULL , `test2` INT NOT NULL );");
        $this->assertEquals(0, self::$connection->errorCode());
    }
